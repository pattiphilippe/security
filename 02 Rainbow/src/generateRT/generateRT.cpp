...
void generateRT(sqlite3 *db, unsigned nbHead, int nbReduce)
{
    ...
    std::ofstream out("rsc/hashToCrack.txt", std::ios_base::out|std::ios_base::trunc); //TODO remove
    std::vector<std::thread> threads;
    for (unsigned i = 0; i < NB_THREADS_GENERATE; i++)
    {
        threads.push_back(std::thread(generateRTInThread, std::reference_wrapper<std::ofstream>(out), db, nbHead / NB_THREADS_GENERATE, nbReduce));
    }
